...
def Potential(j_data, grid_par=-6):
	...
	** Returns **
	...
	print "Rho : ", np.sum(rho)*d3r
	print X[0,0,0], Y[0,0,0], Z[0,0,0]

	## V_r, the contribution from the electron density
	## To avoid calculating the full volume integral,
	## we consider the 1/r factor as a kernel and
	## calculate it statically for X-Y planes, then
	## accumulate across the Z-axis

	## 1. Calculate distance kernel
	X_, Y_ = np.mgrid[ (X[-1,0,0] + 3*X[0,0,0])/4 : (3*X[-1,0,0] + X[0,0,0])/4 : dx,
	                   (Y[0,-1,0] + 3*Y[0,0,0])/4 : (3*Y[0,-1,0] + Y[0,0,0])/4 : dy ]

	#X_ = 1/np.linalg.norm(X_ - 
