...
from orbkit import options, grid, read, main
...
options.itype = "gaussian.log"
options.outputname = argv[4]
options.otype = "cb"
...
try:
	...
	## Negative: the spacing is given and the number of points is deduced
		...
		options.adjust_grid = [over_s, 2.0**(2+p_npts)/3.0   if -5 < p_npts < -1 else \
